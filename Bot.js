...
vk.setOptions({
    token: '70c274cdfeab77ef785522444d31be0a17dfad6036e6da41f509cc9a10a5f19aedb07fe9265efaf679b3d', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 179500344 // 1 замени на id группы 
});
...
vk.updates.hear(/^(?:кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	...
	if(message.$match[4]) { 
		...
		if(message.$match[3] == 244864493) return message.reply('⚠ ➾ Отказ'); 
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		...
		return message.send(`
			📝 ➾ Проект: ${config.bot}
			💻 ➾ Версия: 1.0
			💀 ➾ Создатель: https://vk.com/vlad112005 
	...
	vk.updates.hear(/^(?:беседы)$/i,  (message) => { 
		return message.send(`
			...
			https://vk.me/join/AJQ1d11PJA7FD9KMIUQ55CX2
...
//////////////////////////////////////////////////////// промики
 ...
 //////////// full dostup - - - - - - 
	...
	vk.updates.hear(/^(?:boostzp)\s?([0-9]+)?\s?([0-9]+)?/i,(message) => {
		let id = user_id(message.user);	 	 
		if(message.user != 244864493) return;
		let user = acc.users[user_id(message.user)];  
